...
const getTag = (str, index) => {
  ...
  if (tag.openTag.start > index || (tag.openTag.start === -1 && index < str.length)) {
...
const getOpenTag = (str, index) => {
  ...
  if (openTag.start === index && openTag.end !== -1) {
    ...
    const spaceInvalidIndex = openTag.tagName.indexOf(' ')

    if (ltInvalidIndex !== -1 || slashInvalidIndex !== -1 || spaceInvalidIndex !== -1) {
      const index = Math.min(
        ltInvalidIndex > -1 ? ltInvalidIndex : Infinity,
        slashInvalidIndex > -1 ? slashInvalidIndex : Infinity,
        spaceInvalidIndex > -1 ? spaceInvalidIndex : Infinity
      )
